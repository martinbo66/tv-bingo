  ...
  <div class="shows-container">
    ...
    <div v-if="!loading && !error" class="results-info">
      <span class="results-count">
        Showing {{ filteredShows.length }} of {{ shows.length }} show{{
          shows.length !== 1 ? 's' : ''
        }}
      </span>
    </div>
    ...
    <div v-else-if="error" class="error">
      {{ error }}
      <button class="retry-button" @click="fetchShows">Retry</button>
    </div>
    ...
    <div v-else-if="filteredShows.length === 0" class="no-results">
      ...
      <p v-else>Get started by adding your first show</p>
      <button v-if="hasActiveFilters" class="clear-filters-cta" @click="clearFilters">
        Clear Filters
      </button>
      <router-link v-else to="/create" class="add-show-cta"> + Add Show </router-link>
    </div>
    ...
    <div v-else-if="viewMode === 'grid'" class="shows-grid">
      ...
      >
        <div class="show-content">
          ...
          <div class="phrase-count" :class="`status-${getPhraseCountStatus(show.phrases.length)}`">
            {{ show.phrases.length }} phrase{{ show.phrases.length !== 1 ? 's' : '' }}
            <span v-if="isShowComplete(show)" class="complete-indicator" title="Ready to play!"
              >✓</span
            >
          </div>
        </div>
...
.search-input:focus {
  outline: none;
  background: rgba(255, 255, 255, 0.15);
  border-color: rgba(var(--color-accent-emerald-rgb), 0.6);
  box-shadow: 0 0 0 3px rgba(var(--color-accent-emerald-rgb), 0.2);
}
...
.filter-btn.active {
  background: rgba(var(--color-accent-emerald-rgb), 0.4);
  border-color: rgba(var(--color-accent-emerald-rgb), 0.6);
  color: #fff;
  box-shadow: 0 2px 8px rgba(var(--color-accent-emerald-rgb), 0.3);
}
...
.add-show-cta {
  ...
  background: linear-gradient(
    90deg,
    var(--color-accent-emerald) 0%,
    var(--color-accent-emerald-dark) 100%
  );
  color: #fff;
  border: none;
  border-radius: 24px;
  ...
  cursor: pointer;
  transition: all 0.3s ease;
  box-shadow: 0 4px 15px rgba(var(--color-accent-emerald-rgb), 0.3);
}
...
.add-show-cta:hover {
  background: linear-gradient(
    90deg,
    var(--color-accent-emerald-dark) 0%,
    var(--color-accent-emerald-deep) 100%
  );
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(var(--color-accent-emerald-rgb), 0.4);
}
...
.list-header {
  ...
  gap: 16px;
  padding: 16px 20px;
  background: rgba(var(--color-accent-emerald-rgb), 0.3);
  border-bottom: 2px solid rgba(var(--color-accent-emerald-rgb), 0.5);
  font-weight: 600;
  font-size: 0.9em;
...
.list-row:hover {
  background: rgba(var(--color-accent-emerald-rgb), 0.2);
  border-left: 3px solid rgba(var(--color-accent-emerald-rgb), 0.8);
  padding-left: 17px;
}
